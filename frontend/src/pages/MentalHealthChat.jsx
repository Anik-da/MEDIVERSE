              ...
              {moods.map((m) => {
                ...
                return (
                  <motion.button key={m.label} whileTap={{ scale: 0.9 }} onClick={() => setSelectedMood(m.label)}
                    className={`p-3 rounded-xl border text-center transition-all cursor-pointer ${active ? 'border-opacity-50' : 'border-cyber-border hover:border-opacity-30'}`}
                ...
                return (
                  <div key={d} className="flex flex-col items-center gap-1 flex-1">
                    <motion.div initial={{ height: 0 }} animate={{ height: `${heights[i]}%` }} transition={{ delay: i * 0.08, duration: 0.5 }}
                      className="w-full rounded-t-md bg-gradient-to-t from-neon-pink/40 to-neon-purple/40 min-h-[4px]" />
          ...
          <GlowCard hover={false} glowColor="neon-pink" className="flex flex-col" style={{ minHeight: '520px' }}>
            ...
            <div className="flex-1 overflow-y-auto space-y-4 mb-4 max-h-[420px] pr-2">
              {messages.map((msg, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
                  ...
                  <div className={`max-w-[80%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                    msg.role === 'user'
                      ? 'bg-gradient-to-r from-neon-blue/20 to-neon-purple/20 border border-neon-blue/15 text-text-primary rounded-br-md'
                      : 'bg-cyber-dark border border-cyber-border text-text-secondary rounded-bl-md'
                  }`}>
                    {msg.text}
                  </div>
                </motion.div>
              ...
              {typing && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-start">
                  <div className="bg-cyber-dark border border-cyber-border px-4 py-3 rounded-2xl rounded-bl-md flex items-center gap-1.5">
                    {[0, 1, 2].map(i => (
                      <motion.div key={i} animate={{ y: [0, -6, 0] }} transition={{ duration: 0.6, repeat: Infinity, delay: i * 0.15 }}
                        className="w-2 h-2 rounded-full bg-neon-pink/60" />
                    ))}
                  </div>
                </motion.div>
              )}
            ...
            <div className="flex gap-2 pt-3 border-t border-cyber-border">
              <input value={input} onChange={e => setInput(e.target.value)}
                ...
                className="flex-1 px-4 py-2.5 rounded-xl bg-cyber-dark border border-cyber-border text-sm text-text-primary placeholder-text-muted focus:outline-none focus:border-neon-pink/40 transition-all" />
              <motion.button whileTap={{ scale: 0.9 }} onClick={sendMessage}
                disabled={!input.trim()}
                className="p-2.5 rounded-xl bg-gradient-to-r from-neon-pink to-neon-purple text-white cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed transition-all">
                <Send size={18} />
              </motion.button>
            </div>
          </GlowCard>
